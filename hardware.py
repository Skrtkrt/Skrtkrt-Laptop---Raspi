...
def rotate_left(pca):
    set_pwm(pca, 0, 32768)  # Left wheels backward
    set_pwm(pca, 1, 0)
    set_pwm(pca, 2, 0)      # Right wheels forward
    set_pwm(pca, 3, 32768)

def rotate_right(pca):
    set_pwm(pca, 0, 0)      # Left wheels forward
    set_pwm(pca, 1, 32768)
    set_pwm(pca, 2, 32768)  # Right wheels backward
    set_pwm(pca, 3, 0)
...
def read_temp():
    ...
    if not _temp_sensor_initialized:
        if not initialize_temp_sensor():
            return {'temp': None, 'error': 'Sensor not initialized'}
    ...
    with _temp_sensor_lock:
        try:
            ...
            error_msg = f"Temperature sensor error: {str(e)}"
            print(f"[WARN] {error_msg}")
            return {'temp': None, 'error': error_msg}
